...
// ============================================================================
class jrMonster extends jrPawn
    abstract;
...
{
    return ( (jrMonster(P) != None) && (ClassIsChildOf(Class,P.Class) || ClassIsChildOf(P.Class,Class)) );
}
...
{
    PlaySound(ChallengeSound,SLOT_Talk);
}
...
function PlayTakeHit(vector HitLocation, int Damage, class<DamageType> DamageType)
{
    ...
    PlaySound(HitSound, SLOT_Pain,2*TransientSoundVolume,,400);
}
...
function PlayDyingSound()
{
    ...
    PlaySound(DeathSound, SLOT_Pain,2.5*TransientSoundVolume, true,500);
}
...
function bool MeleeDamageTarget(int hitdamage, vector pushdir)
{
            ...
            <= FMax(CollisionHeight, Controller.Target.CollisionHeight) + 0.5 * FMin(CollisionHeight, Controller.Target.CollisionHeight))) )
    {
        ...
        if ( HitActor != None )
            return false;
        Controller.Target.TakeDamage(hitdamage, self,HitLocation, pushdir, class'jrMeleeDamage');
        return true;
    }
    return false;
}
...
function PlayIdleSound()
{
    ...
    if( FRand() < 0.1 + 0.65*f )
        PlaySound(IdleSound,SLOT_Talk,TransientSoundVolume*(1.0+1.0*f),,TransientSoundRadius*(0.5+1.5*f));
}
...
DefaultProperties
{
    ...
    ControllerClass         = class'JR.jrMonsterController'
    bMeleeFighter           = true
