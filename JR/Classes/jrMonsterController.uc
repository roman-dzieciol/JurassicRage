...
function bool FireWeaponAt(Actor A)
{
    ...
    if ( (A == None) || (Focus != A) )
        return false;
    Target = A;
    jrMonster(Pawn).RangedAttack(Target);
    return false;
}
...
function bool CanAttack(Actor Other)
{
    ...
    return jrMonster(Pawn).CanAttack(Other);
}

function StopFiring()
{
    jrMonster(Pawn).StopFiring();
    bCanFire = false;
    bFire = 0;
    bAltFire = 0;
}
...
function bool FindNewEnemy()
{
    ...
    for( C=Level.ControllerList; C!=None; C=C.NextController )
        if ( C.bIsPlayer && C.Pawn != None && CanSee(C.Pawn) )
        {
            NewDist = VSize(C.Pawn.Location - Pawn.Location);
            if( NewDist < C.Pawn.Region.Zone.DistanceFogEnd * 0.75 )
            {
...
function bool SetEnemy( Pawn NewEnemy, optional bool bHateMonster )
{
    ...
    bNewMonsterEnemy = bHateMonster && (Level.Game.NumPlayers < 4) && !jrMonster(Pawn).SameSpeciesAs(NewEnemy) && !NewEnemy.Controller.bIsPlayer;
    ...
    if ( !bHateMonster && (jrMonster(Enemy) != None) && NewEnemy.Controller.bIsPlayer )
        return false;
...
function bool ClearShot(Vector TargetLoc, bool bImmediateFire)
{
    ...
    if ( !bSeeTarget || !FastTrace(TargetLoc , Enemy.Location + Enemy.BaseEyeHeight * vect(0,0,1)) );
        return false;
    if ( (jrMonster(Pawn).SplashDamage() && (VSize(Pawn.Location - TargetLoc) < jrMonster(Pawn).GetDamageRadius()))
...
function EnemyChanged(bool bNewEnemyVisible)
{
    ...
    if( !Level.Game.bGameEnded ) // hackfix for endgame playsound spam
        jrMonster(Pawn).PlayChallengeSound();
}
...
function WhatToDoNext(byte CallingByte)
{
    ...
    if ( jrMonster(Pawn).bTryToWalk && (Pawn.Physics == PHYS_Flying) && (ChoosingAttackLevel == 0) && (ChooseAttackCounter == 0) )
        TryToWalk();
    ChoosingAttackLevel++;
    ExecuteWhatToDoNext();
    ChoosingAttackLevel--;
...
function ExecuteWhatToDoNext()
{
    ...
    if ( bPreparingMove && jrMonster(Pawn).bShotAnim )
    {
        Pawn.Acceleration = vect(0,0,0);
        jrGotoState('WaitForAnim');
        return;
    }
...
//            }
    ...
    Target = Enemy;
    if( jrMonster(Pawn).PreferMelee() || (bCanCharge && bOldForcedCharge) )
    {
        GoalString = "Charge";
        DoCharge();
        return;
    ...
    if ( !jrMonster(Pawn).PreferMelee() && (FRand() > 0.17 * (skill - 1)) && !DefendMelee(enemyDist) )
    {
    ...
    if ( !jrMonster(Pawn).RecommendSplashDamage() && jrMonster(Pawn).bCanDodge && (FRand() < 0.7) && (FRand()*Skill > 3) )
    {
...
function Restart()
{
    Super.Restart();
    ReSetSkill();
    //jrGotoState('Roaming','DoneRoaming');
    jrGotoState('Waiting');
}
...
function DirectedWander(vector WanderDir)
{
    ...
    if ( TestDirection(WanderDir,Destination) )
        jrGotoState('RestFormation', 'Moving');
    else
        jrGotoState('RestFormation', 'Begin');
}
...
function ReceiveWarning(Pawn shooter, float projSpeed, vector FireDir)
{
    ...
    DodgeSkill = Skill + jrMonster(Pawn).DodgeSkillAdjust;
    if ( (Pawn.health <= 0) || (DodgeSkill < 4) || (Enemy == None) || !jrMonster(Pawn).bCanDodge
...
function bool ShouldStrafeTo(Actor WayPoint)
{
    ...
    if ( jrMonster(Pawn).bAlwaysStrafe )
        return true;
...
{
    if( Pawn.PhysicsVolume.bWaterVolume )
        jrGotoState('GetOutOfWater');
    else
        jrGotoState('RestFormation');
}
...
state RestFormation extends NoGoal
{
    ...
    function PickDestination()
    {
        ...
        {
            f = 1 - jrGameEX(Level.Game).GetDinoRatio();
            f = f**4;
        }

        if( FRand() < f )
        {
            //log(F@jrGameEX(Level.Game).GetDinoRatio());
            N = FindRandomDest();
            A = FindPathToward(N);
...
Moving:
    ...
    PickDestination();
WaitForAnim:
    if ( jrMonster(Pawn).bShotAnim )
    {
        Sleep(0.5);
        Goto('WaitForAnim');
    }
    jrMonster(Pawn).PlayIdleSound();
    if( bAgitated )
    {
        MoveToward(RestPoint,,,True,False);
        if ( Pawn != None )
        {
            Pawn.bStopAtLedges = true;
            Pawn.bAvoidLedges = true;
...
state GetOutOfWater extends NoGoal
{
    ignores EnemyNotVisible;
...
Begin:
    ...
    Sleep(1 + FRand());
Moving:
    MoveTarget = FindPathTowardNearest(class'SmallNavigationPoint');
WaitForAnim:
    if( jrMonster(Pawn).bShotAnim )
    {
        Sleep(0.5);
    ...
    jrMonster(Pawn).PlayIdleSound();
    MoveToward(MoveTarget,,,False,False);
    WhatToDoNext(8);
    Goto('Begin');
}
...
function DoCharge()
{
    ...
    jrGotoState('Charging');
}

function DoTacticalMove()
{
    jrGotoState('TacticalMove');
}
...
ignores SeePlayer, HearNoise;
    ...
    */
    ...
    function bool TryStrafe(vector sideDir)
    {
        ...
        jrGotoState('TacticalMove', 'DoStrafeMove');
        return true;
    }
...
ignores SeePlayer, HearNoise;
    ...
    function EnemyNotVisible()
    {
        ...
        if ( FastTrace(Enemy.Location, LastSeeingPos) )
            jrGotoState('TacticalMove','RecoverEnemy');
        else
            WhatToDoNext(20);
...
DoStrafeMove:
    ...
    if ( (Enemy == None) || EnemyVisible() || !FastTrace(Enemy.Location, LastSeeingPos) || jrMonster(Pawn).PreferMelee() || !Pawn.bCanStrafe )
        Goto('FinishedStrafe');
...
RecoverEnemy:
    ...
    {
        Pawn.Acceleration = vect(0,0,0);
        if ( jrMonster(Pawn).SplashDamage() )
        {
            StopFiring();
            Sleep(0.05);
        }
            ...
            if ( !FastTrace(LastSeenPos, ViewSpot) )
            {
                ...
                if ( PickWallAdjust(Normal(LastSeenPos - ViewSpot)) || FindViewSpot() )
                {
                    if ( Pawn.Physics == PHYS_Falling )
                        ...
                        jrGotoState('Hunting', 'AdjustFromWall');
                }
                else
                {
...
Begin:
    ...
    FinishRotation();
    if ( jrMonster(Pawn).HasRangedAttack() && (FRand() < 0.5) && (VSize(Enemy.Location - FocalPoint) < 150)
...
Begin:
    ...
    if ( Target == Enemy )
        ...
        FireWeaponAt(Target);
    Sleep(0.1);
    if ( jrMonster(Pawn).PreferMelee() || (Target == None) || (Target != Enemy) || jrMonster(Pawn).bBoss )
        WhatToDoNext(35);
    if ( Enemy != None )
        CheckIfShouldCrouch(Pawn.Location,Enemy.Location, 1);
    Focus = Target;
    Sleep(FMax(jrMonster(Pawn).RangedAttackTime(),0.2 + (0.5 + 0.5 * FRand()) * 0.4 * (7 - Skill)));
    WhatToDoNext(36);
    if ( bSoaking )
...
ignores SeePlayer, EnemyNotVisible, HearNoise, ReceiveWarning, NotifyLanded, NotifyPhysicsVolumeChange,
        NotifyHeadVolumeChange, NotifyLanded, NotifyHitWall, NotifyBump, KilledBy, HitWall, Falling, TakeDamage;
...
Begin:
    //xLog( "Begin");
    if( Level.Game.bGameEnded || Level.bStartup || Level.Game.bWaitingToStartMatch )
    {
        Sleep(0.5+0.5*FRand());
        Goto('Begin');
    }
...
final simulated function jrGotoState( name a, optional name b )
{
    //xLog( "jrGotoState" #a #b );
    GotoState(a,b);
}
